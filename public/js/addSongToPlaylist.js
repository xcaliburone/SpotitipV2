document.getElementById('addSongToPlaylistForm').addEventListener('submit', async function(event) {
    ...
    if (!playlistNameDua || !songName) { alert('Please enter both playlist and song names.'); return; }
    ...
    try {
        ...
        if (!playlistIdResponse.ok) { alert('Playlist not found.'); return; }
        ...
        if (addSongToPlaylistResponse.ok) { alert('Song added to playlist successfully!');
