document.getElementById('addSongToAlbumForm').addEventListener('submit', async function(event) {
    ...
    if (!albumNameDua || !songNameDua) { alert('Please enter both playlist and song names.'); return; }
    ...
    try {
        ...
        if (!albumIdResponse.ok) { alert('Playlist not found.'); return; }
        if (!songIdResponse.ok) { alert('Song not found.'); return; }
        ...
        if (addSongToAlbumResponse.ok) { alert('Song added to album successfully!');
        } else { alert('Failed to add song to album.'); }
    } catch (error) {
